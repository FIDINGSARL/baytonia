...
class DispachingBarcodeReport(models.AbstractModel):
    _name = 'report.tracking.barcode.report.xlsx'
    ...
    def generate_xlsx_report(self, workbook, data, wiz):
        ...
        row += 1
        sl_no = 0
        tracking_barcodes = self.env['tracking.barcode'].search([('picking_id', '=', wiz.picking_id.id)])
        for tracking_barcode in tracking_barcodes:
            ...
            worksheet.write(row, 2, tracking_barcode.shipping_company_id.name if tracking_barcode.shipping_company_id else '', data_format)
            worksheet.write(row, 3, tracking_barcode.picking_id.name if tracking_barcode.picking_id else '', data_format)
            worksheet.write(row, 4, tracking_barcode.dispatched_user_id.name if tracking_barcode.dispatched_user_id else '', data_format)
