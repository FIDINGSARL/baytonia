...
class InventoryOverstockReport(models.TransientModel):
    ...
    def generate_on_screen_report(self):
        past_date = fields.Datetime.to_string(fields.datetime.now() - timedelta(days=self.past_sale_dur_days))
        advance_date = fields.Datetime.to_string(fields.datetime.now() + timedelta(days=self.adv_stock_dur_days))
        if self.product_ids:
        ...
        self.env['inventory.overstock.line'].search([]).unlink()
        if self.warehouse_ids and self.product_ids:
        ...
        if get_report_data:
            for product in get_report_data:
                ...
                sale_obj = self.env['sale.order'].search(
                    [('state', 'in', ['sale', 'done']), ('date_order', '>=', past_date),
                     ('order_line.product_id', '=', product.id)])
                purchase_obj = self.env['purchase.order'].search(
                    [('state', 'in', ['purchase', 'done']), ('date_order', '<=', advance_date),
                     ('order_line.product_id', '=', product.id)])
                picking_obj = self.env['stock.picking'].search(
                    [('state', 'in', ['done']), ('date_done', '>=', past_date),
                ...
                if picking_obj:
                    ...
                    line_dict.update({'on_hand_qty': on_hand_qty})
                else:
                    on_hand_qty = product.qty_available
                    line_dict.update({'on_hand_qty': on_hand_qty})
                line_dict.update({'sales_count': product.sales_count})
                if self.past_sale_dur_days and product.sales_count:
                    avg_daily_sale = (product.sales_count / self.past_sale_dur_days)
                    line_dict.update({'avg_daily_sale': round(avg_daily_sale, 2)})
                total_pur_stock = recent_pur_cost = 0
                if purchase_obj:
                    recent_pur_qty = 0
                    ...
                    line_dict.update({
                        'recent_purchase_date': purchase_obj[0].date_order,
                        'recent_purchase_qty': recent_pur_qty,
                        'recent_purchase_cost': recent_pur_cost,
                        'vendor': purchase_obj[0].partner_id.id,
                    })
                ...
                line_dict.update({'stock_coverage': total_pur_stock + total_sale_stock})
                ...
                line_dict.update({
                    ...
                    'overstock_value': overstock_val,
                })

                self.env['inventory.overstock.line'].create(line_dict)

        action = self.env.ref('bi_inventory_overstock_report.action_inventory_overstock_line').read()[0]
        return action
    ...
    def generate_xls_report(self):
        ...
        style_header = xlwt.easyxf(
            "font:height 200; font: name Liberation Sans, bold on,color black; align: horiz center;pattern: pattern solid, fore_colour silver_ega;")
        worksheet = workbook.add_sheet('Sheet 1')
        worksheet.write(3, 0, 'Date', style_table_header)
        worksheet.write(4, 0, str(datetime.date.today()))
        worksheet.write_merge(0, 1, 0, 14, "Inventory Overstock Report", style_title)
        worksheet.write_merge(2, 3, 1, 3, "Last Sale Duration: " + str(self.past_sale_dur_days) + " Days",
                              style=style_table_header)
        worksheet.write_merge(2, 3, 4, 6, "Advance Stock Duration: " + str(self.adv_stock_dur_days) + " Days",
                              style=style_table_header)
        ...
        if get_report_data:
            for product in get_report_data:
                ...
                product_reference = str(product.name) + " " + "[" + str(product.default_code) + "]"
                ...
                if purchase_obj:
                    ...
                    for purchase in purchase_obj:
                        for p_line in purchase.order_line:
                            total_pur_stock = total_pur_stock + p_line.product_qty
                    worksheet.write(row, col + 7, purchase_obj[0].date_order, style)
                ...
                count += 1
                row += 1
        worksheet.write_merge(count + 7, count + 8, 1, 4, "Total Overstock Quantity : " + str(total_overstock_qty),
                              style=style_table_header)
        worksheet.write_merge(count + 7, count + 8, 5, 7, "Total Overstock Value : " + str(total_overstock_val),
                              style=style_table_header)
        worksheet.write_merge(count + 7, count + 8, 8, 10,
                              "Total Duration : " + str(self.past_sale_dur_days + self.adv_stock_dur_days),
                              style=style_table_header)
        worksheet.write_merge(count + 7, count + 8, 11, 13, "Number Of Warehouse : " + str(len(self.get_ware_list())),
                              style=style_table_header)
        fp = io.BytesIO()
        workbook.save(fp)
