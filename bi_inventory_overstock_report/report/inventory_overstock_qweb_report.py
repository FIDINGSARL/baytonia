...
class ReportInventoryCoverage(models.AbstractModel):
    ...
    def get_report_values(self, docids, data=None):
        ...
        past_date = fields.Datetime.to_string(fields.datetime.now() - timedelta(days=int(past_days)))
        advance_date = fields.Datetime.to_string(fields.datetime.now() + timedelta(days=int(advance_days)))
        docs = []
        ...
        for each in report_data_list:
            ...
            picking_obj = self.env['stock.picking'].search([('state', 'in', ['done']),('date_done','>=',past_date),('move_lines.product_id','=',product_id.id)])
            if picking_obj:
            ...
            if purchase_obj:
                for pur in purchase_obj[0]:
                    for pur_line in pur.order_line:
                        ...
                        recent_pur_date = purchase_obj[0].date_order
