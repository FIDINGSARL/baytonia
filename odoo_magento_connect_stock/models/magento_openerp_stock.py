...
class StockMove(models.Model):
        ...
        if 'stock_from' not in ctx and connectionObj:
            for data in self:
                ...
                if flag == 2:
                    ...
                    if not warehouseId or not self.is_mob_warehhouse(warehouseId):
    ...
    @api.model
    def is_mob_warehhouse(self, warehouseId):
        warehouseIds = self.env['magento.configure'].search(
            []).mapped('warehouse_id').ids
        if warehouseId in warehouseIds:
            return True
        return False
        ...
        if mageProductId:
            if not instanceObj.active:
                ...
                try:
                    ...
                    prodResponse = self.env['magento.synchronization'].callMagentoApi(
