...
class SendSupplierStatementReport(models.TransientModel):
    _name = "send.supplier.statement.report"
    ...
    def action_supplier_statement_report_eg(self):
        ...
        for vendor_id in vendor_ids:
            count -= 1
            _logger.info(["==============Cron Supplier Statement report , remaining===", count])
        ...
        attachment_name = "Daily Supplier Statement Report {}".format(date)
        ...
        email_template = self.env['ir.model.data'].get_object(
            'eg_supplier_statement_report', 'email_template_daily_supplier_statement_report')

        values = {
            'subject': "Daily Supplier Statement Report {}".format(date),
            'body': '',
