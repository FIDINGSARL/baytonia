...
class WebsiteSupportSLAResponse(models.Model):
    ...
    @api.depends('category_id', 'response_time')
    def _compute_name(self):
        for sla_response in self:
            if sla_response.category_id and sla_response.response_time:
                sla_response.name = sla_response.category_id.name + " (" + str(sla_response.response_time) + ")"
            else:
                sla_response.name = sla_response.category_id.name
