...
class WebsiteSupportTicket(models.Model):
    ...
    def _default_sla(self):
        default_sla = self.env['website.support.sla'].search([('name', '=', 'Test')])
        if default_sla:
            return default_sla[0]
        else:
            return None
    ...
    sla_id = fields.Many2one('website.support.sla', string="SLA",default=_default_sla)
    ...
    sla_active = fields.Boolean(string="SLA Active", default=True)
    ...
    sla_fail = fields.Boolean(string="SLA Fail", compute="_compute_sla_fail")

    @api.onchange('category')
    def _onchange_category_id(self):
        if self.category:
            response_cat_id = self.env['website.support.sla.response'].search([('category_id', '=', self.category.id)], limit=1)
            self.sla_response_category_id = response_cat_id.id

    @api.one
    @api.depends('sla_timer', 'category', 'sla_id')
    def _compute_sla_fail(self):
        for rec in self:
            for sla_response in rec.sla_id.response_time_ids:
                if sla_response.category_id == rec.category:
                    if -rec.sla_timer > sla_response.response_time:
                        rec.sla_fail = True
    ...
    def update_sla_timer(self):
        ...
        for active_sla_ticket in self.env['website.support.ticket'].search([
            ...
            notification_template = self.env['ir.model.data'].sudo().get_object('website_support',
                                                                                'support_ticket_sla_alert')

            for sla_alert in self.env['website.support.sla.alert'].search(
                    [('vsa_id', '=', active_sla_ticket.sla_id.id), ('alert_time', '>=', active_sla_ticket.sla_timer)]):
    ...
    def _needaction_domain_get(self):
        ...
        custom_replied_state = self.env['ir.model.data'].get_object('website_support',
                                                                    'website_ticket_state_customer_replied')
    ...
    def create(self, vals):
        ...
        if new_id.partner_id.sla_id:
            ...
            category_response = self.env['website.support.sla.response'].search(
        ...
        for my_user in new_id.category.cat_user_ids:
            ...
            values['body_html'] = values['body_html'].replace("_ticket_url_", "web#id=" + str(
                new_id.id) + "&view_type=form&model=website.support.ticket&menu_id=" + str(
    ...
    def write(self, values):
        ...
        for ticket in self:
            if 'state' in values:
                if ticket.state.mail_template_id:
                    ticket.state.mail_template_id.send_mail(ticket.id, True)

            # Email user if category has changed
            if 'category' in values:
                change_category_email = self.env['ir.model.data'].sudo().get_object('website_support',
                                                                                    'new_support_ticket_category_change')
                change_category_email.send_mail(ticket.id, True)

            if 'user_id' in values:
                setting_change_user_email_template_id = self.env['ir.default'].get('website.support.settings',
                ...
                email_values['body_html'] = email_values['body_html'].replace("_user_name_", assigned_user.name if assigned_user.name else '_user_name_' )
                email_values['body'] = email_values['body'].replace("_user_name_", assigned_user.name if assigned_user.name else '_user_name_')
...
class WebsiteSupportTicketCategories(models.Model):
    ...
    access_group_ids = fields.Many2many('res.groups', string="Access Groups",
                                        help="Restrict which users can select the category on the website form, none = everyone")
...
class WebsiteSupportTicketSubCategories(models.Model):
    ...
    additional_field_ids = fields.One2many('website.support.ticket.subcategory.field', 'wsts_id',
...
class WebsiteSupportTicketStates(models.Model):
    ...
    mail_template_id = fields.Many2one('mail.template', domain="[('model_id','=','website.support.ticket')]",
...
class WebsiteSupportTicketClose(models.Model):
    ...
    def close_ticket(self):
        ...
        closed_state = self.env['ir.model.data'].sudo().get_object('website_support',
        ...
        message = "<ul class=\"o_mail_thread_message_tracking\">\n<li>State:<span> " + self.ticket_id.state.name if self.ticket_id.state else '' + " </span><b>-></b> " + closed_state.name + " </span></li></ul>"
        ...
        closed_state_mail_template = self.env['ir.model.data'].get_object('website_support',
...
class WebsiteSupportTicketCompose(models.Model):
    ...
    email_id = fields.Many2one('website.support.ticket.email', string="Email")
    subject = fields.Char(string="Subject", readonly="True")
    body = fields.Text(string="Message Body")
    template_id = fields.Many2one('mail.template', string="Mail Template",
                                  domain="[('model_id','=','website.support.ticket'), ('built_in','=',False)]")
    ...
    def send_reply(self):
        ...
        setting_staff_reply_email_template_id = self.env['ir.default'].get('website.support.settings',
        ...
        values['email_to'] = self.email_id.name
        send_mail = self.env['mail.mail'].create(values)
        send_mail.send()

        # Add to the message history to keep the data clean from the rest HTML
        self.env['website.support.ticket.message'].create({'ticket_id': self.ticket_id.id, 'by': 'staff',
...
class WebsiteSupportTicketEmail(models.Model):
    _name = "website.support.ticket.email"
