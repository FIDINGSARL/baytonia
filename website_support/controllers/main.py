...
class SupportTicketController(http.Controller):
    ...
    def support_subcategories_field_fetch(self, **kwargs):
        ...
        if values['subcategory'] != 'undefined':
            sub_category_fields = request.env['website.support.ticket.subcategory.field'].sudo().search(
                [('wsts_id', '=', int(values['subcategory']))])
        else:
            return ""
        ...
        if sub_category_fields:
            for sub_category_field in sub_category_fields:
                ...
                return_string += "  <label class=\"col-md-3 col-sm-4 control-label\" for=\"efield_" + str(
                    sub_category_field.id) + "\">" + sub_category_field.name + "</label>\n"
    ...
    def support_subcategories_fetch(self, **kwargs):
        ...
        sub_categories = request.env['website.support.ticket.subcategory'].sudo().search(
            [('parent_category_id', '=', int(values['category']))])
        ...
        if sub_categories:
            ...
            for sub_category in request.env['website.support.ticket.subcategory'].sudo().search(
                    [('parent_category_id', '=', int(values['category']))]):
                return_string += "            <option value=\"" + str(
                    sub_category.id) + "\">" + sub_category.name + "</option>\n"
    ...
    def support_help(self, **kw):
        ...
        manager = False
        if request.env['website.support.department.contact'].sudo().search_count(
                [('user_id', '=', request.env.user.id)]) == 1:
            manager = True
    ...
    def support_submit_ticket(self, **kw):
        ...
        ticket_categories = http.request.env['website.support.ticket.categories'].sudo().search(
        ...
        setting_google_captcha_client_key = request.env['ir.default'].get('website.support.settings',
                                                                          'google_captcha_client_key')
        setting_max_ticket_attachments = request.env['ir.default'].get('website.support.settings',
                                                                       'max_ticket_attachments')
        setting_max_ticket_attachment_filesize = request.env['ir.default'].get('website.support.settings',
                                                                               'max_ticket_attachment_filesize')
        setting_allow_website_priority_set = request.env['ir.default'].get('website.support.settings',
                                                                           'allow_website_priority_set')

        return http.request.render('website_support.support_submit_ticket', {'categories': ticket_categories,
                                                                             'priorities': http.request.env[
                                                                                 'website.support.ticket.priority'].sudo().search(
    ...
    @http.route('/support/help/<model("website.support.help.groups"):help_group>', type='http', auth="public",
                website=True)
    ...
    def support_process_ticket(self, **kwargs):
        ...
        values = {}
        print(values, 'processssssssssss')
        for field_name, field_value in kwargs.items():
            values[field_name] = field_value
        print(values, 'processafterrrrrrrrrrrr')
        ...
        if setting_google_recaptcha_active:
            ...
            setting_google_captcha_secret_key = request.env['ir.default'].get('website.support.settings',
        ...
        if "sale_order_id" in values:
            print('orderrrfind')
            #     print(values['order'],'orderrrr')
            sale_order = request.env['sale.order'].sudo().search([('name', '=', values['sale_order_id'])], limit=1)
            if sale_order:
        ...
        print(sale_order.id,sale_id,'sassssssss')

        if http.request.env.user.name != "Public user":
            new_ticket_id = request.env['website.support.ticket'].sudo().create(
                {'person_name': values['person_name'], 'category': values['category'], 'sub_category_id': sub_category,
                 'email': values['email'], 'description': values['description'], 'subject': values['subject'],
                 'partner_id': http.request.env.user.partner_id.id, 'attachment': my_attachment,
                 'attachment_filename': file_name, 'channel': 'Website (User)','sale_order_id':sale_id})
            ...
            if 'priority' in values and (
                    setting_allow_website_priority_set == "partner" or setting_allow_website_priority_set == "everyone"):
            ...
            if len(search_partner) > 0:
                new_ticket_id = request.env['website.support.ticket'].sudo().create(
                    {'person_name': values['person_name'], 'category': values['category'],
                     'sub_category_id': sub_category, 'email': values['email'], 'description': values['description'],
                     'subject': values['subject'], 'attachment': my_attachment, 'attachment_filename': file_name,
                     'partner_id': search_partner[0].id, 'channel': 'Website (Public)','sale_order_id':sale_id})
            else:
                new_ticket_id = request.env['website.support.ticket'].sudo().create(
                    {'person_name': values['person_name'], 'category': values['category'],
                     'sub_category_id': sub_category, 'email': values['email'], 'description': values['description'],
                     'subject': values['subject'], 'attachment': my_attachment, 'attachment_filename': file_name,
                     'channel': 'Website (Public)','sale_order_id':sale_id})
        ...
        if "subcategory" in values:
            ...
            for extra_field in request.env['website.support.ticket.subcategory.field'].sudo().search(
                    [('wsts_id', '=', int(sub_category))]):
                if "efield_" + str(extra_field.id) in values:
                    request.env['website.support.ticket.field'].sudo().create(
                        {'wst_id': new_ticket_id.id, 'name': extra_field.name,
                         'value': values["efield_" + str(extra_field.id)]})
                else:
        ...
        if 'state' in values:
            support_tickets = http.request.env['website.support.ticket'].sudo().search(
                ['|', ('partner_id', '=', http.request.env.user.partner_id.id), ('partner_id', 'in', extra_access),
                 ('partner_id', '!=', False), ('state', '=', int(values['state']))])
        else:
            support_tickets = http.request.env['website.support.ticket'].sudo().search(
                ['|', ('partner_id', '=', http.request.env.user.partner_id.id), ('partner_id', 'in', extra_access),
                 ('partner_id', '!=', False)])
    ...
    def support_portal_ticket_comment(self, **kw):
        ...
        http.request.env['website.support.ticket.message'].sudo().create(
    ...
    def support_ticket_close(self, **kw):
        ...
        if ticket.partner_id.id == http.request.env.user.partner_id.id or ticket.partner_id in http.request.env.user.partner_id.stp_ids:
            ...
            customer_closed_state = request.env['ir.model.data'].sudo().get_object('website_support',
            ...
            diff_time = datetime.datetime.strptime(ticket.close_time,
                                                   DEFAULT_SERVER_DATETIME_FORMAT) - datetime.datetime.strptime(
                ticket.create_date, DEFAULT_SERVER_DATETIME_FORMAT)
