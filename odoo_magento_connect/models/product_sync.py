...
class MagentoSynchronization(models.TransientModel):
                ...
                if not wkAttrLineObjs:
                    ...
                    if not mageSetId:
                        ...
                        if prodResponse and prodResponse.get('id'):
                            ...
                            return [
                                0, str(templateId) + " Error during configurable product sync."]
        else:
            return False
        ...
        if proImage:
            ...
            imageData = {
                ...
                'content': {
                    ...
                    'name': 'ProductImage_'+str(prodObj.id)
                }
            }
            return imageData
        else :
    ...
    def _export_specific_product(self, vrntObj, templateSku, url, token):
        ...
        if vrntObj:
            ...
            getProductData.update(
                ...
                sku=sku,
                price=vrntObj.list_price + vrntObj.price_extra or 0.00,
                custom_attributes=custom_attributes,
                extension_attributes=extension_attributes
            )
    ...
    def _update_specific_product_template(self, mappedObj, url, token):
        ...
        if tempObj and mageProdId:
            if tempObj.product_variant_ids:
                ...
                for vrntObj in tempObj.product_variant_ids:
                    ...
                    if prodMapObj:
                        ...
                        if updtProdIds and not updtProdIds[0]:
                            variant_failed = True
                            response = [0, str(tempObj.id) + ' Failed during varinat update.']
            else:
            ...
            if mappedObj.is_variants and mageProdIds:
                ...
                if not prodResponse:
                    response = [0, str(tempObj.id) + ' Failed during configurable update.']
            if response[0]:
    ...
    def _update_specific_product(self, prodMapObj, url, token):
        ...
        if prodObj and mageProdId:
            ...
            getProductData.update(
                id=mageProdId,
                sku=prodObj.default_code,
                price=prodObj.list_price + prodObj.price_extra or 0.00,
                custom_attributes=custom_attributes
            )
            getProductData = self._get_product_array(
