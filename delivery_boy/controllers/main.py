...
class DeliveryBoyServices(Controller):
	...
	def _authenticate(self, auth, **kwargs):
		...
		if not response.get('success'):
			return response
		request.context = dict(request.context, lang=response.get('lang'), base_url=self.base_url)
		if auth:
			result = delivery_boy_config.authenticate(self._lcred, kwargs.get('detailed',False),self._sLogin, context={'base_url':self.base_url})
			response.update(result)
		return response

	@route('/deliveryBoy/splashPageData', csrf=False, type='http', auth="none", methods=['GET','POST'])
	...
	@route(['/deliveryBoy/pickings/<int:db_picking_id>', '/deliveryBoy/<int:partner_id>/pickings'], csrf=False, type='http', auth="none", methods=['GET','PUT','POST'])
	...
	def _getInvoices(self, partner_id, **kwargs):
		...
		for invoice in account_invoice.search([('partner_id','=',partner_id),('delivery_boy_invoice','=',True)], limit = kwargs.get('limit'), offset = kwargs.get('offset'), order = kwargs.get('order') or 'id desc'):
			invoice_details ={
			'id': invoice.id,
			'name': invoice.number or 'INV',
			'report': [],
			'invoiceDate': invoice.date_invoice.__str__() or '',
	...
	@route('/deliveryBoy/<int:partner_id>', csrf=False, type='http', auth="none", methods=['PUT'])
