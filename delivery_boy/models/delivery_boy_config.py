...
class DeliveryBoyConfig(models.Model):
    ...
    def _getDeliveryBoyProgram(self):
        ...
        self.db_program_id = res_config.get('program', False) and res_config.get('program', False).id
    ...
    def fetch_user_info(self, user_obj, context=None):
        context = context or {}
        temp_i = {
        'deliveryBoyProfileImage': self._get_image_url('res.partner',user_obj.partner_id.id,'image', user_obj.partner_id.write_date.__str__(), context=context),
    ...
    def _getPickingDetails(self, db_picking):
        ...
        'assignedDate':db_picking.assigned_date and datetime.strptime(db_picking.assigned_date, '%Y-%m-%d %H:%M:%S').strftime('%Y-%m-%d') or '',
        'assignedTime':db_picking.assigned_date and datetime.strptime(db_picking.assigned_date, '%Y-%m-%d %H:%M:%S').strftime('%H:%M:%S') or '',
        ...
        if not db_picking.sale_order_id:
            for move in db_picking.picking_id.move_lines:
    ...
    def _productDetails(self, product_id):
        ...
        'attributes':[],
        'image': self._get_image_url('product.product',product_id.id, 'image', product_id.write_date.__str__(), context = {'base_url':self._context.get('base_url')}),
        }
...
class DeliveryBoyPushNotificationTemplate(models.Model):
    ...
    def _send(self, to_data, partner_id=False, db_picking=False, max_limit=20):
        ...
        data_message['image'] = _get_image_url(self._context.get('base_url'), 'delivery.boy.push.notification.template', self.id,'image', self.write_date.__str__())
