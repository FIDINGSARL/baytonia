...
class SetuInventoryTurnoverAnalysisReport(models.TransientModel):
    ...
    partenr_ids = fields.Many2many("res.partner", string="Vendor")
    ...
    def get_inventory_turnover_ratio_data(self):
                ...
                for product_id in products:
                    ...
                    if product:
                        if product.vendor_id:
                            if product.vendor_id.id not in self.partenr_ids.ids:
                                products.remove(product_id)
                        else:
                            products.remove(product_id)
                products = set(products) or {0}
            else:
                products = self.env['product.product'].search([('vendor_id', 'in', self.partenr_ids.ids)])
        ...
        partenr_ids = self.partenr_ids and set(self.partenr_ids.ids) or {}
    ...
    def download_report_in_listview(self):
        ...
        for turnover_data_value in stock_data:
            ...
            product = self.env['product.product'].browse(turnover_data_value.get('product_id'))
            if product:
                turnover_data_value['image_small'] = product.image_small
