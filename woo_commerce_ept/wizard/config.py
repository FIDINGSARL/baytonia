...
class woo_config_settings(models.TransientModel):
    ...
    woo_global_channel_id = fields.Many2one('global.channel.ept',string = "Global Channel")
    ...
    order_import_interval_number = fields.Integer('Import Order Interval Number',help="Repeat every x.",default=10)
    ...
    order_update_interval_number = fields.Integer('Update Order Interval Number',help="Repeat every x.",default=10)
    ...
    order_update_user_id = fields.Many2one('res.users',string="User",help='User',default=lambda self: self.env.user)
