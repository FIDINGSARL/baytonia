...
class woo_product_template_ept(models.Model):
            ...
            for variation_attribute in variation_attributes:
                ...
                if instance.attribute_type=='text':
                    ...
                    if product_attribute:
                        product_attribute_value = product_attribute_value_obj.search([('attribute_id','=',product_attribute.id),('name','=',attribute_val)],limit=1)
                ...
                if instance.attribute_type=='select':
                    ...
                    if woo_product_attribute:
                        woo_product_attribute_term = woo_attribute_term_obj.search([('woo_attribute_id','=',woo_product_attribute.woo_attribute_id),('name','=',attribute_val)],limit=1)
                        if not woo_product_attribute_term:
                            woo_product_attribute_term = woo_attribute_term_obj.search([('woo_attribute_id','=',woo_product_attribute.woo_attribute_id),('slug','=',attribute_val)],limit=1)
    ...
    def create_variant_product(self,result,instance):
        ...
        for attrib in result.get('attributes'):
            ...
            for attrib_vals in attrib_values:
                ...
                if not attrib_value:
                    attrib_value = product_attribute_value_obj.with_context(active_id=False).create({'attribute_id':attribute.id,'name':attrib_vals})                   
    ...
    @api.multi
    def sync_products(self,instance,woo_tmpl_id=False,update_price=False,update_templates=True,sync_images_with_product=False):
        if instance.attribute_type=='select':
            self.sync_woo_attribute(instance)
        woo_product_obj=self.env['woo.product.product.ept']
        transaction_log_obj=self.env["woo.transaction.log"]
    ...
    @api.multi
    def sync_new_products(self,instance,woo_tmpl_id=False,update_price=False,update_templates=True,sync_images_with_product=False):
                    ...
                    for variant in template.woo_product_ids:
                        if variant.variant_id and variant.product_id.type=='product':
                            ...
                            if template.woo_tmpl_id != variant.variant_id:
                                ...
                                flag = True
                            elif template.woo_tmpl_id == variant.variant_id and len(template.woo_product_ids) == 1:
                                del info['variations']
        ...
        for woo_templates in batches:
            ...
            for woo_template in woo_templates:            
                ...
                for variant in woo_template.woo_product_ids:
                    if  variant.variant_id:                                                      
                        price=instance.pricelist_id.get_product_price(variant.product_id,1.0,partner=False,uom_id=variant.product_id.uom_id.id)
                        _logger.info("\n=> pricelist : {} \n=> product_id : {} \n=> price : {} \n product_currency_id : {} \n => pricelist_currency_id : {}".format(instance.pricelist_id.name,variant.product_id,str(price),variant.product_id.currency_id,instance.pricelist_id.currency_id))
                ...
                flag and batch_update_data.append(info)                
            if batch_update_data:
                _logger.info("Batch Update Data : {}".format(batch_update_data))
                batch_update.update({'update':batch_update_data})
                res = wcapi.post('products/batch',batch_update)
    ...
    def get_product_data(self,wcapi,instance,woo_template,publish,update_price,update_stock,update_image,template):
        ...
        data = {'enable_html_description':True,'enable_html_short_description':True,'type': 'simple',
                'title' if old else 'name':woo_template.name,'description':woo_template.description or '','weight':str(template.weight),
...
class woo_product_product_ept(models.Model):
    ...
    url_image=fields.Binary("Image")
                            # ,compute=set_image,store=True)
