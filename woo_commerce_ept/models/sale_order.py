...
class sale_order(models.Model):
    ...
    woo_order_id=fields.Char("Order Reference",help="WooCommerce Order Reference")
    ...
    updated_in_woo=fields.Boolean("Updated In woo")
                                  # ,compute='_get_woo_order_status',search='_search_woo_order_ids',store=True)
    ...
    visible_trans_id=fields.Boolean("trans_id_avail")
...
#                 sale_order.onchange_partner_shipping_id()
    ...
    def auto_update_woo_order_status_ept(self):
        woo_instance_obj=self.env['woo.instance.ept']
        instances = woo_instance_obj.search([()])
        if not isinstance(ctx,dict) or not 'woo_instance_id' in ctx:
            return True
        woo_instance_id = ctx.get('woo_instance_id',False)
        if woo_instance_id:
    ...
    def update_woo_order_status(self,instance):
        ...
        for instance in instances:
            wcapi = instance.connect_in_woo()
            sales_orders = self.search([('warehouse_id','=',instance.warehouse_id.id),('woo_order_id','!=',False),('woo_instance_id','=',instance.id),('updated_in_woo','=',False)],order='date_order')
